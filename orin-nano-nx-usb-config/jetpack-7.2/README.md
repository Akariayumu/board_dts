...
2. 通过Type-C端口连接至Ubuntu主机，使用NVIDIA SDK Manager刷入**JetPack 7.2**系统。
...
3. 设备正常开机后，打开终端命令行，增加**DTB设备树**文件，并修改**extlinux.conf**文件。
   ...
   - **Jetson Orin Nano 8G** 请使用以下代码
     ...
     ```bash
     sudo cp orin_nano_8gb.dtb /boot/dtb
     sudo sed -i 's#console=tty0#console=tty0\n      FDT /boot/dtb/orin_nano_8gb.dtb#g' /boot/extlinux/extlinux.conf
     sudo reboot
     ```
   ...
   >       FDT /boot/dtb/orin_nano_8gb.dtb  #对应dtb文件名
